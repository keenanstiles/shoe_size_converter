...
class Converter:
    ...
    def __init__(self):
        ...
        self.conversion_rates = {
            "US Men's": {
                "EURO Men's": 33,
                "EURO Women's": 2.16667 - 1.5
            },
            "US Women's": {
                "EURO Men's": 2.16667 - 1.5,
                "EURO Women's": 2.16667 - 3
            },
            "EURO Men's": {
                "US Men's": 32 / 5 * 9,
                "US Women's": 32 / 5 * 9 - 1.5
            },
            "EURO Women's": {
                "US Men's": 32 / 5 * 9 + 1.5,
                "US Women's": 32 / 5 * 9 - 1.5
    ...
    def temp_convert(self):
        ...
        if self.from_units.get() == "US":
            if self.to_units.get() == "EURO":
                if self.sizing_var.get() == "Men's":
                    answer = (to_convert) + self.conversion_rates["US Men's"]["EURO Men's"]
                    from_to = "{} US Men's is {} EURO Men's"

                elif self.sizing_var.get() == "Women's":
                    answer = (to_convert) + self.conversion_rates["US Women's"]["EURO Women's"]
                    from_to = "{} US Women's is {} EURO Women's"

            elif self.to_units.get() == "US":
                if self.sizing_var.get() == "Men's":
                    answer = to_convert
                    from_to = "{} US Men's is {} US Men's"

                elif self.sizing_var.get() == "Women's":
                    answer = to_convert
                    from_to = "{} US Women's is {} US Women's"

        elif self.from_units.get() == "EURO":
            if self.to_units.get() == "US":
                if self.sizing_var.get() == "Men's":
                    answer = (to_convert) + self.conversion_rates["EURO Men's"]["US Men's"]
                    from_to = "{} EURO Men's is {} US Men's"

                elif self.sizing_var.get() == "Women's":
                    answer = (to_convert) + self.conversion_rates["EURO Women's"]["US Women's"]
                    from_to = "{} EURO Women's is {} US Women's"

            elif self.to_units.get() == "EURO":
                if self.sizing_var.get() == "Men's":
                    answer = to_convert
                    from_to = "{} EURO Men's is {} EURO Men's"

                elif self.sizing_var.get() == "Women's":
                    answer = to_convert
                    from_to = "{} EURO Women's is {} EURO Women's"
