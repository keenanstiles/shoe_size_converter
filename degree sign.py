...
class Converter:
    ...
    def __init__(self, partner):
        ...
        instructions = "Please enter a temperature below and " \
                       "then select the units to convert to."
        ...
        self.from_units.set("Celsius")  # default value

        self.to_units = StringVar()
        self.to_units.set("Fahrenheit")  # default value

        self.dropdown_frame = Frame(self.temp_frame)
        self.dropdown_frame.grid(row=4)

        self.from_dropdown = OptionMenu(self.dropdown_frame, self.from_units, "Celsius", "Fahrenheit")
        self.from_dropdown.config(font=dropdown_font)
        self.from_dropdown.grid(row=0, column=0, padx=5, pady=5)

        self.to_dropdown = OptionMenu(self.dropdown_frame, self.to_units, "Celsius", "Fahrenheit")
    ...
    def temp_convert(self, min_val):
        ...
        if self.from_units.get() == "Celsius" and self.to_units.get() == "Fahrenheit":
            answer = to_convert * 1.8 + 32
            from_to = "{} C{} is {} F{}"

        elif self.from_units.get() == "Fahrenheit" and self.to_units.get() == "Celsius":
