...
class DisplayHelp:
    ...
    def __init__(self):
        ...
        instructions = "To use the program, simply enter the shoe size value " \
                       ...
                       "Note that your entered value must exceed 4 US (or equivalent)\n\n" \
                       "To see your " \
        ...
        self.to_converter_button = Button(self.help_frame,
                                          ...
                                          font=button_font,
                                          width=20,
                                          command=self.bye_window)
        self.to_converter_button.grid(row=2, padx=5, pady=5)

        # actuates the minimising of help and simultaneous opening of converter class
    def bye_window(self):
        # Minimize the help window
        self.help_frame.master.withdraw()
        # Open the converter window and pass a reference to DisplayHelp instance
        Converter(self)


class Converter:
    # display_help_instance is present so class can accept bye_window reference as an argument
    def __init__(self, display_help_instance):
        self.display_help_instance = display_help_instance
        ...
        # Conversion rates that will be acessed specifically in relation to dropdown and
    ...
    def close_convert(self):
        # Destroy the converter window
        self.converter_box.destroy()
        # Un-minimize the help window
        self.display_help_instance.help_frame.master.deiconify()
    ...
    def check_shoe(self, min_value):
        try:
            ...
            if response < min_value:
                ...
                self.output_label.config(text=f"Please enter a number greater than {min_value}", fg="#9C0000")
                self.var_feedback.set(f"Please enter a number greater than {min_value}")
                self.output_answer()
                return None
            else:
            ...
            self.output_label.config(text="Please enter a valid number", fg="#9C0000")
            self.output_answer()
            return None

    # output answer determines the presence of error inputs with check_shoe, then acorrdingly
    # alters entry box and other frontdoor features to exemplify an incorrect input
    def output_answer(self):
        ...
        if has_errors == "yes":
            ...
            self.output_label.config(fg="#004C00", font=("Arial", "10"))
    ...
    def shoe_convert(self):
        # another error def, focuses on wether the output answer is tangible
        # eliminates outputs like "-23 US Men's" and instead runs and error code
        error = "Please enter a valid integer greater than 4 US, \n" \
                "or equivalent value"
        ...
        if to_convert is not None:
            ...
            if (to_units == "US" and converted_size < 4) or \
                    (to_units == "EURO" and converted_size < 35) or \
                    (to_units == "UK" and converted_size < 3.5):
                error_message = f"Converted size is less than 4 US, \n" \
                                " or equivalent value"
                self.output_label.config(text=error_message, fg="#9C0000")
                self.shoe_entry.config(bg="#F8CECC")
                self.var_feedback.set(error_message)
            else:
                self.output_label.config(text=message, fg="#228b22")
...
class HistoryExport:
    ...
    def __init__(self, partner, calc_list):
        ...
        hist_text = "{}  \n\nAll calculations are shown in " \
                    "chronological order.".format(showing_all)
